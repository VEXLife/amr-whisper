...
class SignalFeatureExtractor:
    ...
    def __call__(self, iq_wave):
        ...
        iq_wave = torch.nn.functional.pad(
            iq_wave, (0, self.max_seq_len - iq_wave.shape[1]), mode='constant', value=0)
        return iq_wave
...
class SignalLogitsProcessor(LogitsProcessor):
    def __init__(self):
        ...
        self.symb_seq_mask = torch.tensor(
            [i + vocab['0'] for i in range(32)], dtype=torch.long)
        ...
        for i in range(pred_logits.shape[0]):
            ...
            for j in range(pred_logits.shape[1]):
                pred_logits_new = self.logits_processor_list(
                    torch.tensor(pred_seq_ids), pred_logits[i, j].unsqueeze(0))
